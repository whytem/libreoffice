...
- also builds in standalone mode from `spreadsheet_engine/` alone
...
- Transitional rule:
  ...
  - Calc should move to the copied implementation under
    `spreadsheet_engine/` as soon as practical
...
- Extraction Phase 0 completed:
  - `Module_spreadsheet_engine.mk` and `Library_spreadsheetengine.mk` exist
...
- Standalone Phase A completed:
  ...
  - standalone shim headers exist under `shims/include/`
...
- Phase 6 is now substantially complete for the portable helper lane:
  ...
  - a first shared parity seed now exists at
    `tests/parity/numeral_conversion_cases.tsv`
  ...
  - a shared parity seed now also exists at
    `tests/parity/workday_cases.tsv`
  ...
  - a shared parity seed now also exists at
    `tests/parity/calendar_cases.tsv`
  ...
  - a shared parity seed now also exists at `tests/parity/text_cases.tsv`
...
- Phase 7 started:
  ...
  - the first compiler-support standalone assertions now run in
    `tests/unit/compiler_api_tests.cxx`
...
- Standalone gate:
  - `cmake -S spreadsheet_engine -B /tmp/spreadsheetengine-standalone-build`
...
- completed
- implemented in:
  - `spreadsheet_engine/inc/spreadsheetengine/api/Host.hxx`
  - `spreadsheet_engine/inc/spreadsheetengine/runtime/InMemoryHost.hxx`
  - `spreadsheet_engine/inc/spreadsheetengine/compat/libreoffice/Host.hxx`
  - `spreadsheet_engine/inc/spreadsheetengine/detail/HostValueAccess.hxx`
  - `spreadsheet_engine/tests/unit/host_api_tests.cxx`
...
- pass 1: pure reference and result carriers
  ...
  - copy these into engine-owned headers and implementations under
    `spreadsheet_engine`, then turn the Calc-facing versions into thin
...
- pass 1 is now substantially complete:
  - the engine owns standalone-facing carrier headers for formula results and
    references in:
    - `spreadsheet_engine/inc/spreadsheetengine/api/FormulaResult.hxx`
    - `spreadsheet_engine/inc/spreadsheetengine/api/ReferenceData.hxx`
  ...
  - the Calc build graph now consistently propagates
    `spreadsheet_engine/inc` into the targets that include `sc/inc/refdata.hxx`
...
- pass 5 is now substantially complete:
  ...
  - Phase 11 is now complete:
    - `spreadsheet_engine/` builds and installs as a real standalone package
...
- standalone:
  - clean standalone build from `spreadsheet_engine/` alone
...
- `spreadsheet_engine/` builds as a genuine standalone package
...
1. Use `./spreadsheet_engine/tools/run_maintenance_validation.sh` for the default
   dual-run maintenance gate.
2. Use `./spreadsheet_engine/tools/run_maintenance_validation.sh --milestone` for
   the broader stable Calc closeout subset.
3. Use `./spreadsheet_engine/integration/libreoffice/run_spreadsheet_unit_tests.sh --engine` when a
