    ...
    const std::string aResultMessage
        = "unexpected computational lifecycle result kind="
          ...
          + (oResult->moGraphComparison
                 ? std::to_string(static_cast<int>(oResult->moGraphComparison->meKind))
                       + ":" + (oResult->moGraphComparison->mbFullMatch ? "1" : "0")
                       + ":" + (oResult->moGraphComparison->mbFormulaNodeMatch ? "1" : "0")
                       + ":" + (oResult->moGraphComparison->mbFormulaGroupNodeMatch ? "1" : "0")
                       + ":" + (oResult->moGraphComparison->mbListenerAnchorMatch ? "1" : "0")
                       + ":" + (oResult->moGraphComparison->mbBroadcasterNodeMatch ? "1" : "0")
                       + ":" + (oResult->moGraphComparison->mbEdgeMatch ? "1" : "0")
                 : std::string("none"))
          + " ir="
          + (oResult->moIrComparison
                 ? std::to_string(static_cast<int>(oResult->moIrComparison->meKind))
                       + ":" + (oResult->moIrComparison->mbFullMatch ? "1" : "0")
                       + ":" + (oResult->moIrComparison->mbSnapshotMatch ? "1" : "0")
                       + ":" + (oResult->moIrComparison->mbGrammarMatch ? "1" : "0")
                       + ":" + (oResult->moIrComparison->mbFormulaRecordNormalizedMatch ? "1" : "0")
                       + ":" + (oResult->moIrComparison->mbFormulaGroupNormalizedMatch ? "1" : "0")
                       + ":" + (oResult->moIrComparison->mbBuildFailureNormalizedMatch ? "1" : "0")
                 : std::string("none"));
    CPPUNIT_ASSERT_MESSAGE(
        aResultMessage,
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityEnabledByCandidateGate)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetScalarValue;
    ...
    const CellAddress aExpectedAnchor { 0, 1, 1 };
    CPPUNIT_ASSERT(aAfterGroups.front().maAnchor == aExpectedAnchor);
    ...
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityNamedRangeMemberExitSetFormulaApplies)
{
    ...
    m_pDoc->SetString(1, 0, 0, u"=COUNTA(Metrics)+A1*10"_ustr);
    forceFormulaTreeOrder(*m_pDoc,
        { ScAddress(1, 0, 0), ScAddress(1, 1, 0), ScAddress(1, 2, 0), ScAddress(2, 0, 0) });

    const CalcWorkbookFacade aMutationFacade(*m_pDoc, 1);
    const auto aBoundary = consumers::classifySharedFormulaNamedRangeMutationBoundary(
        aBeforeFacade, aMutationFacade,
        spreadsheetengine::detail::facade::MutationEvent::setFormula(
            { 0, 1, 0 }, u"=COUNTA(Metrics)+A1*10"));
    CPPUNIT_ASSERT_EQUAL(
        consumers::SharedFormulaNamedRangeMutationBoundary::GlobalSingleAreaSameSheet,
        aBoundary.meBoundary);

    const auto oResult = aAuthorityCapture.apply(
        *m_pDoc, translateSetFormula(ScAddress(1, 0, 0), u"=COUNTA(Metrics)+A1*10"_ustr));
    assertComputationalPilotApplied(oResult, *m_pDoc);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityOffSheetConsumerMemberExitApplies)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetScalarValue;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    m_pDoc->SetValue(1, 1, 0, 99.0);
    forceFormulaTreeOrder(*m_pDoc,
        { ScAddress(1, 0, 0), ScAddress(1, 2, 0), ScAddress(2, 0, 1) });

    const auto oResult
        = aAuthorityCapture.apply(*m_pDoc, translateSetScalarValue(ScAddress(1, 1, 0)));
    assertComputationalPilotApplied(oResult, *m_pDoc);

    const CalcWorkbookFacade aAfterFacade(*m_pDoc, 1);
    const auto aAfterGroups = consumers::collectFormulaGroupDescriptors(aAfterFacade);
    CPPUNIT_ASSERT(aAfterGroups.empty());
    CPPUNIT_ASSERT(!m_pDoc->GetFormulaCell(ScAddress(1, 1, 0)));
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityOffSheetConsumerMemberExitSetFormulaApplies)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    m_pDoc->SetString(1, 1, 0, u"=A2*10"_ustr);
    forceFormulaTreeOrder(*m_pDoc,
        { ScAddress(1, 0, 0), ScAddress(1, 1, 0), ScAddress(1, 2, 0), ScAddress(2, 0, 1) });

    const auto oResult
        = aAuthorityCapture.apply(*m_pDoc, translateSetFormula(ScAddress(1, 1, 0), u"=A2*10"_ustr));
    assertComputationalPilotApplied(oResult, *m_pDoc);

    const CalcWorkbookFacade aAfterFacade(*m_pDoc, 1);
    const auto aAfterGroups = consumers::collectFormulaGroupDescriptors(aAfterFacade);
    CPPUNIT_ASSERT(aAfterGroups.empty());
    ScFormulaCell* pEditedFormula = m_pDoc->GetFormulaCell(ScAddress(1, 1, 0));
    CPPUNIT_ASSERT(pEditedFormula);
    CPPUNIT_ASSERT_EQUAL(u"=A2*10"_ustr, pEditedFormula->GetFormula());
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityOffSheetConsumerMemberExitClearCellApplies)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateClearCell;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aAuthorityCapture.apply(*m_pDoc, translateClearCell(ScAddress(1, 1, 0)));
    assertComputationalPilotApplied(oResult, *m_pDoc);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityOffSheetConsumerSameTextPreserveApplies)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult
        = aAuthorityCapture.apply(*m_pDoc, translateSetFormula(ScAddress(1, 1, 0), u"=A2*2"_ustr));
    assertComputationalPilotApplied(oResult, *m_pDoc);

    const CalcWorkbookFacade aAfterFacade(*m_pDoc, 1);
    const auto aClassification = consumers::classifySharedFormulaMutation(
        aBeforeFacade, aAfterFacade,
        spreadsheetengine::detail::facade::MutationEvent::setFormula({ 0, 1, 1 }, u"=A2*2"));
    ...
    CPPUNIT_ASSERT_EQUAL(u"=Data.B1+Data.B2+Data.B3"_ustr, pSummaryFormula->GetFormula());
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityOffSheetConsumerRegroupApplies)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult
        = aAuthorityCapture.apply(*m_pDoc, translateSetFormula(ScAddress(1, 1, 0), u"=A2*3"_ustr));
    assertComputationalPilotApplied(oResult, *m_pDoc);
    ...
    CPPUNIT_ASSERT((aAfterGroups.front().maAnchor == CellAddress { 0, 1, 0 }));
    CPPUNIT_ASSERT_EQUAL(static_cast<sal_Int32>(2), aAfterGroups.front().mnLength);
    ScFormulaCell* pTailFormula = m_pDoc->GetFormulaCell(ScAddress(1, 2, 0));
    CPPUNIT_ASSERT(pTailFormula);
    CPPUNIT_ASSERT_EQUAL(u"=A3*2"_ustr, pTailFormula->GetFormula());
    ScFormulaCell* pSummaryFormula = m_pDoc->GetFormulaCell(ScAddress(2, 0, 1));
    CPPUNIT_ASSERT(pSummaryFormula);
    CPPUNIT_ASSERT_EQUAL(u"=Data.B1+Data.B2+Data.B3"_ustr, pSummaryFormula->GetFormula());
    ...
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleOffSheetConsumerMemberExitSetFormulaApplies)
{
    ...
    const auto oResult = aLifecycleCapture.apply(
        *m_pDoc, translateSetFormula(ScAddress(1, 1, 0), u"=A2*10"_ustr));
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleOffSheetConsumerSameTextPreserveApplies)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aLifecycleCapture.apply(
        *m_pDoc, translateSetFormula(ScAddress(1, 1, 0), u"=A2*2"_ustr));
    assertComputationalLifecycleApplied(oResult, *m_pDoc);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleOffSheetConsumerRegroupApplies)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aLifecycleCapture.apply(
        *m_pDoc, translateSetFormula(ScAddress(1, 1, 0), u"=A2*3"_ustr));
    assertComputationalLifecycleApplied(oResult, *m_pDoc);
    ...
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleOffSheetConsumerMergeApplies)
{
    ...
    const auto oResult = aLifecycleCapture.apply(
        *m_pDoc, translateSetFormula(ScAddress(1, 2, 0), u"=A3*2"_ustr));
    assertComputationalLifecycleApplied(oResult, *m_pDoc);
    CPPUNIT_ASSERT(oResult->moComputationalComparison.has_value());
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleOffSheetConsumerReplacementMergeApplies)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    m_pDoc->SetString(1, 2, 0, u"=A3*2"_ustr);
    m_pDoc->SetString(1, 3, 0, u"=A4*2"_ustr);
    m_pDoc->SetString(2, 0, 1, u"=Data.B1+Data.B2+Data.B3+Data.B4"_ustr);
    m_pDoc->CalcAll();
    ...
    forceFormulaTreeOrder(*m_pDoc,
        { ScAddress(1, 0, 0), ScAddress(1, 1, 0), ScAddress(1, 2, 0),
            ScAddress(1, 3, 0), ScAddress(2, 0, 1) });

    const auto oResult = aLifecycleCapture.apply(
        *m_pDoc, translateSetFormula(ScAddress(1, 2, 0), u"=A3*3"_ustr));
    assertComputationalLifecycleApplied(oResult, *m_pDoc);
    CPPUNIT_ASSERT(oResult->moComputationalComparison.has_value());
    CPPUNIT_ASSERT(oResult->moComputationalComparison->mbGroupMatch);
    CPPUNIT_ASSERT(oResult->moComputationalComparison->mbFullMatch);
    ...
    ScFormulaCell* pSummaryFormula = m_pDoc->GetFormulaCell(ScAddress(2, 0, 1));
    CPPUNIT_ASSERT(pSummaryFormula);
    CPPUNIT_ASSERT_EQUAL(u"=Data.B1+Data.B2+Data.B3+Data.B4"_ustr,
        pSummaryFormula->GetFormula());
    ScFormulaCell* pResidualFormula = m_pDoc->GetFormulaCell(ScAddress(1, 3, 0));
    CPPUNIT_ASSERT(pResidualFormula);
    CPPUNIT_ASSERT_EQUAL(u"=A4*2"_ustr, pResidualFormula->GetFormula());
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityNamedRangeOffSheetMemberExitApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetScalarValue;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    m_pDoc->SetValue(1, 0, 0, 9.0);
    forceFormulaTreeOrder(*m_pDoc,
        { ScAddress(1, 1, 0), ScAddress(1, 2, 0), ScAddress(0, 0, 1) });
    ...
    CPPUNIT_ASSERT((aAfterGroups.front().maAnchor == CellAddress { 0, 1, 1 }));
    CPPUNIT_ASSERT_EQUAL(static_cast<sal_Int32>(2), aAfterGroups.front().mnLength);
    CPPUNIT_ASSERT_EQUAL(2.0, m_pDoc->GetValue(ScAddress(0, 0, 1)));
    CPPUNIT_ASSERT(!m_pDoc->GetFormulaCell(ScAddress(1, 0, 0)));
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityNamedRangeOffSheetMemberExitSetFormulaApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    m_pDoc->SetString(1, 0, 0, u"=COUNTA(Metrics)+A1*10"_ustr);
    forceFormulaTreeOrder(*m_pDoc,
        { ScAddress(1, 0, 0), ScAddress(1, 1, 0), ScAddress(1, 2, 0), ScAddress(0, 0, 1) });
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityNamedRangeOffSheetMemberExitClearCellApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateClearCell;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityNamedRangeOffSheetRegroupApplies)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aAuthorityCapture.apply(
        *m_pDoc, translateSetFormula(ScAddress(1, 1, 0), u"=COUNTA(Metrics)+A2*3"_ustr));
    assertComputationalPilotApplied(oResult, *m_pDoc);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityNamedRangeOffSheetOneSidedInsertApplies)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aAuthorityCapture.apply(
        *m_pDoc, translateSetFormula(ScAddress(1, 2, 0), u"=COUNTA(Metrics)+A3*2"_ustr));
    assertComputationalPilotApplied(oResult, *m_pDoc);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleNamedRangeOffSheetSameTextPreserveApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleNamedRangeOffSheetRegroupApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleNamedRangeOffSheetOneSidedInsertApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleNamedRangeOffSheetMemberExitSetFormulaApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleNamedRangeOffSheetMemberExitClearCellApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateClearCell;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleOffSheetConsumerMemberExitClearCellApplies)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateClearCell;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aLifecycleCapture.apply(*m_pDoc, translateClearCell(ScAddress(1, 1, 0)));
    assertComputationalLifecycleApplied(oResult, *m_pDoc);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralOffSheetConsumerMemberExitAuthorityApplies)
{
    ...
    CPPUNIT_ASSERT(aEntryCapture.canApplyMutationEntry());

    const auto oResult = aEntryCapture.apply(*m_pDoc,
        MutationEntryRequest::setScalarValue({ 0, 1, 1 }, CellValue::number(99.0)));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);

    const CalcWorkbookFacade aAfterFacade(*m_pDoc, 1);
    const auto aAfterGroups = consumers::collectFormulaGroupDescriptors(aAfterFacade);
    CPPUNIT_ASSERT(aAfterGroups.empty());
    CPPUNIT_ASSERT_EQUAL(107.0, m_pDoc->GetValue(ScAddress(2, 0, 1)));
    CPPUNIT_ASSERT(!m_pDoc->GetFormulaCell(ScAddress(1, 1, 0)));
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralOffSheetConsumerMemberExitLifecycleSetFormulaApplies)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 1 }, u"=A2*10"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
    ...
    CPPUNIT_ASSERT(pEditedFormula);
    CPPUNIT_ASSERT_EQUAL(u"=A2*10"_ustr, pEditedFormula->GetFormula());
    CPPUNIT_ASSERT_EQUAL(28.0, m_pDoc->GetValue(ScAddress(2, 0, 1)));
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralOffSheetConsumerMemberExitLifecycleClearCellApplies)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult
        = aEntryCapture.apply(*m_pDoc, MutationEntryRequest::clearCell({ 0, 1, 1 }));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
    ...
    CPPUNIT_ASSERT(aAfterGroups.empty());
    CPPUNIT_ASSERT(!m_pDoc->GetFormulaCell(ScAddress(1, 1, 0)));
    CPPUNIT_ASSERT_EQUAL(8.0, m_pDoc->GetValue(ScAddress(2, 0, 1)));
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralOffSheetConsumerSameTextPreserveLifecycleApplies)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 1 }, u"=A2*2"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
    ...
    CPPUNIT_ASSERT((aAfterGroups.front().maAnchor == CellAddress { 0, 1, 0 }));
    CPPUNIT_ASSERT_EQUAL(static_cast<sal_Int32>(3), aAfterGroups.front().mnLength);
    CPPUNIT_ASSERT_EQUAL(12.0, m_pDoc->GetValue(ScAddress(2, 0, 1)));
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralOffSheetConsumerRegroupLifecycleApplies)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 1 }, u"=A2*3"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
    ...
    CPPUNIT_ASSERT(pTailFormula);
    CPPUNIT_ASSERT_EQUAL(u"=A3*2"_ustr, pTailFormula->GetFormula());
    CPPUNIT_ASSERT_EQUAL(15.0, m_pDoc->GetValue(ScAddress(2, 0, 1)));
    ...
    testComputationalMutationEntrySharedGroupNonStructuralOffSheetConsumerMergeLifecycleApplies)
{
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 2 }, u"=A3*2"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
    CPPUNIT_ASSERT(oResult->moComputationalComparison.has_value());
    CPPUNIT_ASSERT(oResult->moComputationalComparison->mbGroupMatch);
    CPPUNIT_ASSERT(oResult->moComputationalComparison->mbFullMatch);
    ...
    CPPUNIT_ASSERT((aAfterGroups.front().maAnchor == CellAddress { 0, 1, 0 }));
    CPPUNIT_ASSERT_EQUAL(static_cast<sal_Int32>(5), aAfterGroups.front().mnLength);
    CPPUNIT_ASSERT_EQUAL(30.0, m_pDoc->GetValue(ScAddress(2, 0, 1)));
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralOffSheetConsumerReplacementMergeLifecycleApplies)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 2 }, u"=A3*3"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
    CPPUNIT_ASSERT(oResult->moComputationalComparison.has_value());
    CPPUNIT_ASSERT(oResult->moComputationalComparison->mbGroupMatch);
    CPPUNIT_ASSERT(oResult->moComputationalComparison->mbFullMatch);
    ...
    CPPUNIT_ASSERT((aAfterGroups.front().maAnchor == CellAddress { 0, 1, 0 }));
    CPPUNIT_ASSERT_EQUAL(static_cast<sal_Int32>(3), aAfterGroups.front().mnLength);
    CPPUNIT_ASSERT_EQUAL(26.0, m_pDoc->GetValue(ScAddress(2, 0, 1)));
    ScFormulaCell* pResidualFormula = m_pDoc->GetFormulaCell(ScAddress(1, 3, 0));
    CPPUNIT_ASSERT(pResidualFormula);
    CPPUNIT_ASSERT_EQUAL(u"=A4*2"_ustr, pResidualFormula->GetFormula());
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralNamedRangeOffSheetSameTextPreserveApplies)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 1 }, u"=COUNTA(Metrics)+A2"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralNamedRangeOffSheetRegroupApplies)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 1 }, u"=COUNTA(Metrics)+A2*3"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralNamedRangeOffSheetOneSidedInsertApplies)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 2 }, u"=COUNTA(Metrics)+A3*2"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralNamedRangeOffSheetMemberExitAuthorityApplies)
{
    using spreadsheetengine::api::CellValue;
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aEntryCapture.apply(*m_pDoc,
        MutationEntryRequest::setScalarValue({ 0, 1, 0 }, CellValue::number(9.0)));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralNamedRangeOffSheetMemberExitLifecycleSetFormulaApplies)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 0 }, u"=COUNTA(Metrics)+A1*10"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralNamedRangeOffSheetMemberExitLifecycleClearCellApplies)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityStaysRejectedWithoutCandidateGate)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetScalarValue;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleEnabledByCandidateGate)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    m_pDoc->SetString(1, 1, 0, u"=A2*5"_ustr);
    forceFormulaTreeOrder(*m_pDoc,
        { ScAddress(1, 0, 0), ScAddress(1, 1, 0), ScAddress(1, 2, 0) });

    const auto oResult = aLifecycleCapture.apply(
        *m_pDoc, translateSetFormula(ScAddress(1, 1, 0), u"=A2*5"_ustr));
    assertComputationalLifecycleApplied(oResult, *m_pDoc);

    const CalcWorkbookFacade aAfterFacade(*m_pDoc, 1);
    const auto aAfterGroups = consumers::collectFormulaGroupDescriptors(aAfterFacade);
    CPPUNIT_ASSERT(aAfterGroups.empty());
    ScFormulaCell* pFormula = m_pDoc->GetFormulaCell(ScAddress(1, 1, 0));
    CPPUNIT_ASSERT(pFormula);
    CPPUNIT_ASSERT_EQUAL(u"=A2*5"_ustr, pFormula->GetFormula());

    m_pDoc->DeleteTab(0);
}

CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleSameTextPreserve)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityNamedRangeRegroupApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    ScFormulaCell* pTailFormula = m_pDoc->GetFormulaCell(ScAddress(1, 2, 0));
    CPPUNIT_ASSERT(pTailFormula);
    CPPUNIT_ASSERT_EQUAL(u"=COUNTA(Metrics)+A3*2"_ustr, pTailFormula->GetFormula());

    m_pDoc->DeleteTab(0);
}

CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityNamedRangeOneSidedInsertApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    testComputationalNarrowRolloutSharedGroupNonStructuralAuthorityNamedRangeSplitOutcomeApplies)
{
    ...
    m_pDoc->SetString(1, 4, 0, u"=COUNTA(Metrics)+A5*3"_ustr);
    m_pDoc->SetString(1, 5, 0, u"=COUNTA(Metrics)+A6*3"_ustr);
    m_pDoc->SetString(2, 0, 0, u"=COUNTA(Metrics)"_ustr);
    m_pDoc->CalcAll();
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleNamedRangeRegroupApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleNamedRangeOneSidedInsertApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleRegroup)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleMerge)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    ...
    forceFormulaTreeOrder(*m_pDoc,
        { ScAddress(1, 0, 0), ScAddress(1, 1, 0), ScAddress(1, 2, 0),
            ScAddress(1, 3, 0), ScAddress(1, 4, 0) });
    ...
    const CalcWorkbookFacade aAfterFacade(*m_pDoc, 1);
    const auto aAfterGroups
        = spreadsheetengine::detail::facade::consumers::collectFormulaGroupDescriptors(aAfterFacade);
    CPPUNIT_ASSERT_EQUAL(static_cast<std::size_t>(1), aAfterGroups.size());
    CPPUNIT_ASSERT((aAfterGroups.front().maAnchor == CellAddress { 0, 1, 0 }));
    CPPUNIT_ASSERT_EQUAL(static_cast<sal_Int32>(5), aAfterGroups.front().mnLength);
    CPPUNIT_ASSERT(oResult->moComputationalComparison.has_value());
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleReplacementMerge)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleOneSidedInsert)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleThreeGroupAttemptKeepsFarGroupSeparate)
{
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    m_pDoc->SetString(1, 2, 0, u"=A3*2"_ustr);
    m_pDoc->SetString(1, 3, 0, u"=A4*2"_ustr);
    m_pDoc->SetString(1, 4, 0, u"=A5*3"_ustr);
    m_pDoc->SetString(1, 5, 0, u"=A6*3"_ustr);
    m_pDoc->CalcAll();
    ...
    forceFormulaTreeOrder(*m_pDoc,
        { ScAddress(1, 0, 0), ScAddress(1, 1, 0), ScAddress(1, 2, 0),
            ScAddress(1, 3, 0), ScAddress(1, 4, 0), ScAddress(1, 5, 0) });
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleNonEdgeRegroupNormalizesToMemberExit)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto aAfterGroups = consumers::collectFormulaGroupDescriptors(aAfterFacade);
    CPPUNIT_ASSERT(aAfterGroups.empty());
    ScFormulaCell* pTouchedFormula = m_pDoc->GetFormulaCell(ScAddress(1, 2, 0));
    CPPUNIT_ASSERT(pTouchedFormula);
    CPPUNIT_ASSERT_EQUAL(u"=A3*3"_ustr, pTouchedFormula->GetFormula());

    m_pDoc->DeleteTab(0);
}

CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupNonStructuralLifecycleNonEdgeMergeNormalizesToMemberExit)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateSetFormula;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    m_pDoc->SetString(1, 4, 0, u"=A5*2"_ustr);
    m_pDoc->SetString(1, 5, 0, u"=A6*3"_ustr);
    m_pDoc->SetString(1, 6, 0, u"=A7*3"_ustr);
    m_pDoc->CalcAll();
    ...
    m_pDoc->SetString(1, 3, 0, u"=A4*3"_ustr);
    forceFormulaTreeOrder(*m_pDoc,
        { ScAddress(1, 0, 0), ScAddress(1, 1, 0), ScAddress(1, 2, 0),
            ScAddress(1, 3, 0), ScAddress(1, 4, 0), ScAddress(1, 5, 0), ScAddress(1, 6, 0) });

    const auto oResult = aLifecycleCapture.apply(
        *m_pDoc, translateSetFormula(ScAddress(1, 3, 0), u"=A4*3"_ustr));
    assertComputationalLifecycleApplied(oResult, *m_pDoc);
    ...
    CPPUNIT_ASSERT((aAfterGroups.front().maAnchor == CellAddress { 0, 1, 0 }));
    CPPUNIT_ASSERT_EQUAL(static_cast<sal_Int32>(2), aAfterGroups.front().mnLength);
    CPPUNIT_ASSERT((aAfterGroups.back().maAnchor == CellAddress { 0, 1, 5 }));
    CPPUNIT_ASSERT_EQUAL(static_cast<sal_Int32>(2), aAfterGroups.back().mnLength);
    ScFormulaCell* pTouchedFormula = m_pDoc->GetFormulaCell(ScAddress(1, 3, 0));
    CPPUNIT_ASSERT(pTouchedFormula);
    CPPUNIT_ASSERT_EQUAL(u"=A4*3"_ustr, pTouchedFormula->GetFormula());
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralAuthority)
{
    using spreadsheetengine::api::CellValue;
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralLifecycle)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 1 }, u"=A2*5"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
    CPPUNIT_ASSERT(oResult->moComputationalComparison.has_value());
    CPPUNIT_ASSERT(oResult->moComputationalComparison->mbGroupMatch);
    CPPUNIT_ASSERT(oResult->moComputationalComparison->mbFullMatch);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralSameTextPreserveLifecycle)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralNamedRangeRegroupApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    testComputationalMutationEntrySharedGroupNonStructuralNamedRangeSplitOutcomeNormalizesToRegroupApplies)
{
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 2 }, u"=COUNTA(Metrics)+A3*3"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
    CPPUNIT_ASSERT(oResult->moComputationalComparison.has_value());
    CPPUNIT_ASSERT(oResult->moComputationalComparison->mbFullMatch);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralNamedRangeOneSidedInsertApplies)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralRegroupLifecycle)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralMergeLifecycle)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralReplacementMergeLifecycle)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    CPPUNIT_ASSERT((aAfterGroups.front().maAnchor == CellAddress { 0, 1, 1 }));
    CPPUNIT_ASSERT_EQUAL(static_cast<sal_Int32>(3), aAfterGroups.front().mnLength);
    ScFormulaCell* pResidualFormula = m_pDoc->GetFormulaCell(ScAddress(1, 0, 0));
    CPPUNIT_ASSERT(pResidualFormula);
    CPPUNIT_ASSERT_EQUAL(u"=A1*2"_ustr, pResidualFormula->GetFormula());

    m_pDoc->DeleteTab(0);
}

CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralOneSidedInsertLifecycle)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 0 }, u"=A1*2"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
    CPPUNIT_ASSERT(oResult->moComputationalComparison.has_value());
    CPPUNIT_ASSERT(oResult->moComputationalComparison->mbGroupMatch);
    CPPUNIT_ASSERT(oResult->moComputationalComparison->mbFullMatch);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralThreeGroupAttemptKeepsFarGroupSeparate)
{
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralNonEdgeRegroupNormalizesToMemberExit)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalMutationEntrySharedGroupNonStructuralNonEdgeMergeNormalizesToMemberExit)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::detail::substrate::MutationEntryRequest;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    const auto oResult = aEntryCapture.apply(
        *m_pDoc, MutationEntryRequest::setFormula({ 0, 1, 3 }, u"=A4*3"));
    assertComputationalMutationEntryApplied(oResult, *m_pDoc);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupPreserveEnabledByCandidateGate)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateInsertRows;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    CPPUNIT_ASSERT(oResult->moComputationalComparison->mbFullMatch);
    CPPUNIT_ASSERT(oResult->moGraphComparison.has_value());
    CPPUNIT_ASSERT_EQUAL(GraphComparisonKind::Exact, oResult->moGraphComparison->meKind);
    CPPUNIT_ASSERT(oResult->moIrComparison.has_value());
    ...
    const auto aAfterGroups = consumers::collectFormulaGroupDescriptors(aAfterFacade);
    CPPUNIT_ASSERT_EQUAL(static_cast<std::size_t>(1), aAfterGroups.size());
    const spreadsheetengine::api::CellAddress aExpectedAnchor { 0, 1, 1 };
    CPPUNIT_ASSERT(aExpectedAnchor == aAfterGroups.front().maAnchor);
    CPPUNIT_ASSERT_EQUAL(static_cast<sal_Int32>(2), aAfterGroups.front().mnLength);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupSplitEnabledByCandidateGate)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateInsertRows;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    m_pDoc->InsertRow(ScRange(0, 1, 0, m_pDoc->MaxCol(), 1, 0));
    forceFormulaTreeOrder(*m_pDoc, { ScAddress(1, 0, 0), ScAddress(1, 2, 0) });
    ...
    const auto aGroupTransition = consumers::classifyFormulaGroupTransition(
        ...
        spreadsheetengine::detail::facade::MutationEvent::insertRows(0, 1, 1));
    CPPUNIT_ASSERT_EQUAL(
        spreadsheetengine::detail::facade::consumers::SharedFormulaGroupTransitionKind::Split,
    ...
    const CalcWorkbookFacade aVerifiedFacade(*m_pDoc, 1);
    const auto aVerifiedGroups = consumers::collectFormulaGroupDescriptors(aVerifiedFacade);
    CPPUNIT_ASSERT(aVerifiedGroups.empty());

    m_pDoc->DeleteTab(0);
}

CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalNarrowRolloutSharedGroupRebuildEnabledByCandidateGate)
{
    using spreadsheetengine::compat::libreoffice::CalcWorkbookFacade;
    using spreadsheetengine::compat::libreoffice::mutation::translateInsertRows;
    namespace consumers = spreadsheetengine::detail::facade::consumers;
    ...
    m_pDoc->InsertRow(ScRange(0, 1, 0, m_pDoc->MaxCol(), 1, 0));
    forceFormulaTreeOrder(*m_pDoc, { ScAddress(1, 0, 0), ScAddress(1, 2, 0), ScAddress(1, 3, 0) });
    ...
    const auto aGroupTransition = consumers::classifyFormulaGroupTransition(
        ...
        spreadsheetengine::detail::facade::MutationEvent::insertRows(0, 1, 1));
    CPPUNIT_ASSERT_EQUAL(
        spreadsheetengine::detail::facade::consumers::SharedFormulaGroupTransitionKind::Rebuild,
        aGroupTransition.meKind);
    ...
    const CalcWorkbookFacade aVerifiedFacade(*m_pDoc, 1);
    const auto aVerifiedGroups = consumers::collectFormulaGroupDescriptors(aVerifiedFacade);
    CPPUNIT_ASSERT_EQUAL(static_cast<std::size_t>(1), aVerifiedGroups.size());
    const spreadsheetengine::api::CellAddress aExpectedAnchor { 0, 1, 2 };
    CPPUNIT_ASSERT(aExpectedAnchor == aVerifiedGroups.front().maAnchor);
    CPPUNIT_ASSERT_EQUAL(static_cast<sal_Int32>(2), aVerifiedGroups.front().mnLength);
...
CPPUNIT_TEST_FIXTURE(TestDependencyShadow,
    testComputationalObjectRealizationSharedGroupFormulaGroupAnchorsRemoveExactRestore)
{
    using spreadsheetengine::compat::libreoffice::bootstrapMutableComputationalSubstrateState;
    using spreadsheetengine::compat::libreoffice::makeComputationalObservationState;
    using spreadsheetengine::compat::libreoffice::recalcqueue::captureFormulaState;
    ...
    CPPUNIT_ASSERT_EQUAL_MESSAGE(describeObjectRealizationObservation(aObjectObservation),
        ObjectRealizationObservationKind::Exact, aObjectObservation.meKind);
    CPPUNIT_ASSERT(aObjectObservation.maReason.empty());
